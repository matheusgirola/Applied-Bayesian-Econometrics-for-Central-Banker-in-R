clear
addpath('functions');
data=xlsread('\data\tvardata2.xlsx');
L=2;  %lag length
tard=2;  %delay
tarvar=4;  % threshold variable is the column number tarvar in data
MaxTrys=1000;
...
%append
  ...
  disp(sprintf(' Replication %s of %s acceptance %s. ', ... 
         ...
         if igibbs>BURN
             ...
             A01=chol(sigma1);
             A02=chol(sigma2);
             irf1=irfsim(reshape(beta1,N*L+1,N),N,L,A01,[0 0 0 1],HORZ);
             irf2=irfsim(reshape(beta2,N*L+1,N),N,L,A02,[0 0 0 1],HORZ);
             irf1=irf1./irf1(1,4);
             irf2=irf2./irf2(1,4);
  ...
  figure(1)
  TT=1973.75:0.25:2014;
  subplot(5,2,1)
  plot(TT,[mean(smat==1,2) Ystar])
  axis tight
  legend('S_{t}>Y^{*}','Threshold Variable')
  title('Probability of Regime 2')
  subplot(5,2,2)
  plot(TT,[repmat(mean(tmat),T,1) Ystar])
  axis tight
  legend('Y^{*}','Threshold Variable')
  title('Threshold')
  ...
  subplot(5,2,9);
  plot(prctile(irfmat1(:,:,4),[50 16 84],1)')
  title('Regime 1');
  ylabel('NFCI');
  axis tight
  subplot(5,2,10);
  plot(prctile(irfmat2(:,:,4),[50 16 84],1)')
  title('Regime 2');
