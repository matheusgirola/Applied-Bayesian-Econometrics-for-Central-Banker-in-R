...
#
# The code is similar to example6, the difference is the algorithm to search the
# sign restriction matrix, A0, generates 100 matrices that satisfies the sign
# restriction or each Gibbs draw, and retains the A0 matrix closest to the mean
# or median of these 100 matrices
#
# This is what they called "example7.m"
#
...
for (i in 1:REPS){
  ...
  if (i > BURN){
    ...
    for (M in 1:100){
      ...
      A0big = rbind(A0big, matrix(A0new, nrow = 1))
    }
    MA0 = median(A0big) # find the median
    DIFFA0 = rowSums((A0big - matrix(MA0, nrow = 100, ncol = ncol(A0big)))^2) # distance from the median
    id = which(DIFFA0 == min(DIFFA0), arr.ind = TRUE)[1] #return row of min value
    A0NEW = matrix(A0big[id,], nrow = N, ncol = N) # A0 matrix closest to the median
