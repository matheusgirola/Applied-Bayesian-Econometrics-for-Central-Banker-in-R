...
# This is what they called "examplem2.m"
...
j=1
for (i in list(B_sample[,1], B_sample[,2], B_sample[,3], sigma_sample)){
  model <- lm (i~1)
...
rownames(summary) = c("alpha", "Beta 1", "Beta 2", "sigma")
summary

# Plot of the forecast distribution
library(fanplot)
plot(data$Inflation[1:250], type = "l", xlim = c(0, 270))
fan(data =  forecast, start = 240, p =seq(0.05, 0.95, 0.05), 
